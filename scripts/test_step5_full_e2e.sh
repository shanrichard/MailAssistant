#!/bin/bash
# Test Step 5: 完整的端到端测试所有修复

echo "=== Step 5 测试：完整的端到端测试 ==="
echo "测试所有修复是否协同工作正常"

# 配置
export TEST_USER_ID="60f2ccbd-d754-4fa0-aa4d-35a7d6551d38"
export TEST_USER_EMAIL="single.shan@gmail.com"
export API_BASE="http://localhost:8000/api/v1"

# 创建主测试脚本
cat > test_e2e_sync.py << 'EOF'
#!/usr/bin/env python3
"""完整的端到端测试脚本"""
import asyncio
import requests
...
# 运行完整测试
echo -e "\n运行端到端测试..."
python test_e2e_sync.py
TEST_RESULT=$?

# 清理
rm -f test_e2e_sync.py get_test_token.py

# 结果
echo -e "\n=== Step 5 测试结果 ==="
if [ $TEST_RESULT -eq 0 ]; then
    echo "✅ Step 5 端到端测试全部通过！"
    echo ""
    echo "所有修复验证完成："
