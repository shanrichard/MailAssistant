...
### 1. search_gmail_online - 直连 Gmail 搜索 【优先使用】
- **默认首选工具**：能搜索 Gmail 中的所有邮件，包括所有分类
- 搜索范围最全面，包括推广、社交、动态、论坛等所有分类的邮件
- 使用 Gmail 搜索语法：`search_gmail_online(query="from:google.com newer_than:7d")`
- 最多返回40封邮件，但覆盖范围最广

### 2. search_email_history - 本地数据库搜索 【补充工具】
- 速度快，支持分页，可获取更多邮件
- 仅搜索已同步到本地的邮件（可能不完整）
...
1. **首次搜索**：优先使用 `search_gmail_online`
   - 能确保搜索到所有分类的邮件
   - 特别适合搜索推广邮件、社交邮件等可能被本地同步遗漏的内容

2. **需要更多结果时**：补充使用 `search_email_history`
   - 当Gmail在线搜索返回结果较少且需要更多邮件时
   - 需要分页获取大量邮件进行详细分析时

3. **特殊情况**：
   - 用户明确要求"详细分析"或"获取所有相关邮件"时，两个工具都使用
...
1. **立即尝试Gmail在线搜索** - 如果刚才使用的是本地搜索
   - 使用 `search_gmail_online` 直接搜索Gmail服务器
   - 能找到所有分类中的邮件，包括推广、社交等

2. **调整搜索关键词** - 如果在线搜索也没结果
   - 分析用户搜索词，尝试相关变体
   - 使用更宽泛的搜索条件

3. **分析 sender_summary** - 展示实际存在的发件人
4. **解释可能原因** - 帮助用户理解搜索结果
5. **建议优化搜索策略** - 指导用户如何调整
