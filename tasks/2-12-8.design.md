...
1. **日报时间设置（唯一功能）**
   ...
   - 时间以用户本地时区为准
   - 设置后立即生效，更新后端定时任务
   - **注意**：手动触发日报生成功能属于DailyReport页面，不在Settings页面
...
<Settings>
  ...
  {/* 注意：手动生成日报功能应该在DailyReport页面，不在Settings页面 */}
</Settings>
```
...
export const schedulerApi = {
  ...
  updateSchedule: (data: ScheduleUpdate) => api.post('/scheduler/schedule', data), // 注意：后端实际使用POST，非PUT
  // triggerDailyReport应该在dailyReportService中，不在这里
};
...
class ScheduleUpdate(BaseModel):
    daily_report_time: str  # HH:mm 格式
    timezone: str           # IANA时区
    auto_sync_enabled: bool = True
    
    @validator('daily_report_time')
...
@router.post("/schedule")  # 后端实际使用POST而非PUT
